...
replace("const res=await fetch(url,{headers:{accept:'application/json'}});", `const reply=await CapacitorHttp.get({url:new URL(url,${JSON.stringify(u.origin)}).href,headers:{accept:'application/json'},responseType:'text',connectTimeout:15000,readTimeout:60000});
...
console.log('Bundled v0.7.6 for',u.origin);
